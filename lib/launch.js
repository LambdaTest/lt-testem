...
function launch(options) {
  ...
  if (!auth.username || !auth.accessKey) {
    var msg = "Please, provide the username and the access key for LambdaTest. Use the option --help for more information.";
    ...
    return Bluebird.using(tunnel(config), function () {
      return Bluebird.using(remoteBrowser(desired, auth, integrationTestURL), function (browser) {
        ...
        return new Bluebird.Promise(function (resolve) {
          ...
          var url = "https://automation.lambdatest.com/test?testID=" + browser.sessionID;
          ...
          console.log("# Updating lambdatest build status");
          var auth = config.auth();
